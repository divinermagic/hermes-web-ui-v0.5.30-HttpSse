...
目标很明确：**聊天、群聊、看板等核心实时能力不再依赖 Socket.IO 长连接，而是改为 HTTP 请求 + SSE（Server-Sent Events）事件流。**
...
- 原 Socket.IO `/socket.io/` 通道不作为核心实时通道使用。
- 冒烟测试中 `/socket.io/` 返回 `404`，证明核心实时链路没有走 Socket.IO 服务。
- WebSocket Upgrade 已做拦截处理，避免误走 Socket.IO 通道。

### 已知说明

- 本仓库仍可能保留部分历史依赖、测试文件或终端相关代码中的 `socket.io` / `ws` 字符串，这是上游 v0.5.30 历史代码和兼容残留，不代表核心 ChatRun / 群聊 / 看板链路仍依赖 Socket.IO。
- 如果在强禁止 WebSocket 的环境中部署，请优先验证 ChatRun、群聊、看板这三条 HTTP/SSE 主链路；Web Terminal 等特殊功能如仍使用 PTY/WebSocket，应单独评估或关闭。
...
POST /api/hermes/chat-run/actions/run       # 启动 ChatRun
POST /api/hermes/chat-run/actions/abort     # 中止 ChatRun
...
GET  /api/hermes/kanban/events              # 看板 SSE 事件流
```
...
本地已有冒烟测试报告：[`sse-conversion-smoke-test-report.md`](./sse-conversion-smoke-test-report.md)
...
4. 看板事件 SSE 正常。
5. 反代没有缓冲或中断 `text/event-stream`。
6. 如所在网络完全禁止 WebSocket，请确认不启用仍依赖 WebSocket 的特殊功能。
