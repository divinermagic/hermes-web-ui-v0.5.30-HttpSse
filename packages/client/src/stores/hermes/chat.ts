import { startRunViaSocket, resumeSession, registerSessionHandlers, unregisterSessionHandlers, respondToolApproval, type RunEvent, type ContentBlock as ContentBlockImport } from '@/api/hermes/chat'
...
export const useChatStore = defineStore('chat', () => {
  ...
  function removeQueuedMessage(sessionId: string, messageId: string) {
    ...
    fetch(`${getBaseUrlValue()}/api/hermes/chat-run/cancel-queued?token=${encodeURIComponent(getApiKey())}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId, queue_id: messageId }),
    }).catch(() => {})
  }
  ...
  function resumeServerWorkingRun(sid: string) {
    ...
    streamStates.value.set(sid, {
      abort: () => {
        fetch(`${getBaseUrlValue()}/api/hermes/chat-run/abort?token=${encodeURIComponent(getApiKey())}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
