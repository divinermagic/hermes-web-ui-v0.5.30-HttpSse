...
export const useKanbanStore = defineStore('kanban', () => {
  ...
  function connectEventStream(board: string, generation: number, seq: number) {
    ...
    let es: EventSource
    try {
      es = kanbanApi.openKanbanEventStream({ board })
    } catch (err) {
    ...
    es.onerror = () => {
      if (eventSource === es) console.error('Kanban event stream error')
    }

    // Poll for close since EventSource has no native onclose
