...
import { emitToSession as sseEmitToSession } from './sse-manager'
...
export async function handleAbort(
  ...
  bridge: any,
  runQueuedItem: (socket: any, sessionId: string, next: QueuedRun, fallbackProfile?: string) => void,
) {
...
export async function markAbortCompleted(
  ...
  const emit = (event: string, payload: any) => {
    if (nsp) {
      ...
      sseEmitToSession(sessionId, event, payload)
    }
  }
...
function emitToSession(nsp: any, socket: any, sessionId: string, event: string, payload: any) {
  ...
  if (nsp) {
    ...
    sseEmitToSession(sessionId, event, tagged)
  }
}
