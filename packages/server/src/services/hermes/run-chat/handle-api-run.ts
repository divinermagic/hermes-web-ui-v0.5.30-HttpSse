...
export async function handleApiRun(
  ...
  dequeueNextQueuedRun: (socket: any, sessionId: string, fallbackProfile?: string) => void,
) {
  ...
  if (session_id) {
    ...
    if (socket) socket.join(`session:${session_id}`)
  }
  ...
  const emit = (event: string, payload: any) => {
    ...
    if (session_id && nsp) {
      ...
      sseEmitToSession(session_id, event, tagged)
    }
  }
  try {
