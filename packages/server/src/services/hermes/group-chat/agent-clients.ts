...
// Lazy require for io to avoid crash when socket.io-client isn't installed
let _io: any = null
try {
  _io = require('socket.io-client').io
} catch { /* SSE mode — socket.io-client not installed */ }
...
class AgentClient {
    ...
    async connect(port?: number): Promise<void> {
        ...
        this.socket = _io(`http://127.0.0.1:${actualPort}/group-chat`, {
            auth: {
