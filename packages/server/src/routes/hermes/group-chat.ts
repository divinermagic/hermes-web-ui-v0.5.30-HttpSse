...
import type { SseGroupChatServer } from '../../services/hermes/group-chat/sse-server'

export const groupChatRoutes = new Router()

let chatServer: GroupChatServer | null = null
let sseServer: SseGroupChatServer | null = null

export function setGroupChatServer(server: GroupChatServer | SseGroupChatServer) {
    chatServer = server as GroupChatServer
}

export function getGroupChatServer(): GroupChatServer | SseGroupChatServer | null {
    return (sseServer || chatServer) as any
}
...
groupChatRoutes.get('/api/hermes/group-chat/events', async (ctx) => {
    const server = sseServer
    if (!server) {
        ctx.status = 503
        ctx.body = { error: 'SSE transport not enabled' }
        return
    }
    await server.handleEventsStream(ctx)
})

// ─── SSE Action Routes (POST) ──────────────────────────────

groupChatRoutes.post('/api/hermes/group-chat/send', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleMessage(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/join-room', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleJoin(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/message-stream-start', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleMessageStreamStart(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/message-stream-delta', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleMessageStreamDelta(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/message-reasoning-delta', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleMessageReasoningDelta(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/message-stream-end', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleMessageStreamEnd(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/typing', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleTyping(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/stop-typing', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleStopTyping(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/context-status', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleContextStatus(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/interrupt-agent', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleInterruptAgent(ctx)
})

groupChatRoutes.post('/api/hermes/group-chat/approval-respond', async (ctx) => {
    const server = sseServer
    if (!server) { ctx.status = 503; ctx.body = { error: 'SSE transport not enabled' }; return }
    await server.handleApprovalRespond(ctx)
})
