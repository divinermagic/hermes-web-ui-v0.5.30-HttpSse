...
import { kanbanEventsRoutes } from './routes/hermes/kanban-events'
import { startVersionCheck } from './routes/health'
import { registerRoutes } from './routes'
import { setSseGroupChatServer } from './routes/hermes/group-chat'
import { setSseChatRunServer } from './routes/hermes/chat-run'
import { SseGroupChatServer } from './services/hermes/group-chat/sse-server'
import { SseChatRun } from './services/hermes/run-chat/sse-chat-run'
...
let sseGroupChatServer: SseGroupChatServer | null = null
...
export async function bootstrap() {
  ...
  // Terminal WebSocket (deleted as-is — user requirement)
  setupTerminalWebSocket(servers)
  console.log('[bootstrap] terminal websocket setup')

  // Kanban SSE events (replaces WebSocket)
  app.use(kanbanEventsRoutes.routes())
  app.use(kanbanEventsRoutes.allowedMethods())
  console.log('[bootstrap] kanban SSE events setup')

  // Group chat SSE server (replaces Socket.IO)
  ...
  if (sseGroupChatServer) {
    sseGroupChatServer.restoreWhenReady()
  }
  ...
  bindShutdown(servers, sseGroupChatServer, sseChatRunServer, agentBridgeManager)
  startVersionCheck()
}
